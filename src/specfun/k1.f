...
C
C     BESK1(ARG)  XLEAST < ARG < XMAX    K1(ARG)          1
C     BESEK1(ARG)     XLEAST < ARG       EXP(ARG)*K1(ARG)    2
C
...
C Error returns
C
C  The program returns the value XINF for ARG <= 0.0 and the
C   BESK1 entry returns the value 0.0 for ARG > XMAX.
C
C
...
CS    REAL
      DOUBLE PRECISION
     1    ARG,F,G,P,PP,Q,QQ,RESULT,SUMF,SUMG,
     2    SUMP,SUMQ,X,XINF,XMAX,XLEAST,XSMALL,XX
...
CS   1     XMAX/85.343E+0/
     ...
     1     XMAX/705.343D+0/
C--------------------------------------------------------------------
C  Coefficients for  XLEAST <=  ARG  <= 1.0
C--------------------------------------------------------------------
